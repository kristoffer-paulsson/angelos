...
from angelos.archive7.fs import TYPE_FILE, TYPE_DIR, TYPE_LINK, EntryRecord
from angelos.lib.error import ArchiveInvalidSeek
            ...
            if os.path.isfile(source):
                ...
                for filename in glob.iglob(
                    ...
                    if os.path.isfile(filename):
                        ...
                        try:
                            arch.mkdir(
                                dirname, user=user, group=group, perms=perms
                            )
                        except RuntimeError:
                            arch.chmod(
                                dirname,
                ...
                raise OSError("%s is not a valid file or directory" % source)
