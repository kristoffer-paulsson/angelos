...
import asyncssh
from angelos.lib.error import CmdShellException, CmdShellEmpty, CmdShellInvalidCommand, CmdShellExit, Error
from angelos.lib.ioc import ContainerAware
from angelos.common.utils import Util, FactoryInterface
...
class Command(FactoryInterface):
    ...
    def _evaluate(self, opts):
        ...
        if opts:
            raise Error.exception(Error.CMD_OPT_UNKNOWN, {"unkown": opts})
...
class Shell(ContainerAware):
    ...
    async def execute(self, line):
        ...
        if bool(line) is False:
            raise Error.exception(Error.CMD_SHELL_EMPTY)

        cmd = re.findall(self.cmd_regex, line)
        if len(cmd) is not 1:
            raise Error.exception(Error.CMD_SHELL_CONFUSED, {"line": line})
        cmd = cmd[0]
