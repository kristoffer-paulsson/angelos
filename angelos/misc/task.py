...
class Signal:
    ...
    def __init__(self, halt, run):
        ...
        """
        Util.is_type(halt, threading.Event)
        Util.is_type(run, threading.Event)
...
class Monitor:
    ...
    def state(self, state=None):
        Util.is_type(state, (str, type(None)))
...
class TaskGroup:
    ...
    def stop(self):
        ...
        up all the threads.
        """
        logger.info(Util.format_info('Halting group', {'group': self.__name}))
        self.__halt.set()
        self.__run.set()
...
class TaskManager:
    ...
    def __init__(self, groups, runlevels, ioc):
        Util.is_type(groups, list)
        Util.is_type(runlevels, list)
        Util.is_type(ioc, Container)
        ...
        for pkg in groups:
            try:
                self.__classes.append(Util.imp_pkg(pkg))
            except ModuleNotFoundError:  # noqa F821 (Linter missconfiguration)
                raise Util.exception(Error.TASKGROUP_NOT_FOUND, {'path': pkg})

    def initialize(self, klass):
        Util.is_class(klass, TaskGroup)
    ...
    def stop(self):
        ...
        up all the threads.
        """
        logger.info(Util.format_info('Halting all threads'))
        ...
        for i in range(TaskManager.KILL_RANGE):
            if threading.active_count() > 1:
                ...
                logger.info(Util.format_info('All threads halted'))
                return True
        logger.error(Util.format_info('Couldn\'t halt all threads'))
        return False
