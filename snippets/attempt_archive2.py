...
class Archive(ContainerAware):
    ...
    def create(path, owner, node=None, title=None,
               network=None, _type=None, role=None, use=None, secret=None):
        header = Header.header(
            owner=owner, node=node, title=title,
            network=network, _type=_type, role=role, use=use)

        if secret:
            with ConcealIO(path, 'wb', secret) as cnl:
                cnl.write(header.serialize())
                for i in range(header.entries):
                    cnl.write(Entry.blank().serialize())
                cnl.seek(0)

            fileobj = ConcealIO(path, 'rb+', secret)
            return Archive(fileobj)
        else:
            with open(path, 'wb') as cnl:
                cnl.write(header.serialize())
                for i in range(header.entries):
                    cnl.write(Entry.blank().serialize())
                cnl.seek(0)
    ...
    class Operations(ContainerAware):
        ...
        def write_data(self, offset, data):
            fileobj = self.ioc.fileobj
            hit = fileobj.seek(offset)
            if hit != offset:
                print('Failed seek:', offset, hit)
                raise OSError('Could not seek to new offset')
            fileobj.write(data)
...
if encrypted:
    archive = Archive.create(
        path='./test.ar7.cnl',
        owner=uuid.UUID(bytes=b'\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x00'),  # noqa
...
for i in range(random.randrange(100, 200)):
    filename = random.choices(animals, k=1)[0] + '/' + ''.join(
        random.choices(
...
if encrypted:
    archive = Archive.open(
        path='./test.ar7.cnl', secret=base64.b64decode(secret))
else:
    archive = Archive.open(path='./test.ar7')
