        ...
        for iteration in range(m_height):
            ...
            for block in range(m_width):
                ...
                composite = Eidon.compose(
                    streamed, q_size, Eidon.Palette.RGB676)
    ...
    def quantize(matrix, factor):
        for y in range(8):
            for x in range(8):
                v = matrix[y][x]
                if v > 0:
                    v = math.log(v) * Eidon.LOG_NORM
                elif v < 0:
                    v = math.log(abs(v)) * Eidon.LOG_NORM * -1
                matrix[y][x] = round(v)
        return matrix
    ...
    def dequantize(matrix, factor):
        for y in range(8):
            for x in range(8):
                v = matrix[y][x]
                if v > 0:
                    v = math.exp(v / Eidon.LOG_NORM)
                elif v < 0:
                    v = math.exp(abs(v) / Eidon.LOG_NORM) * -1
                matrix[y][x] = round(v)
        return matrix
