...
class ConcealIO(io.RawIOBase):
    ...
    def __length(self, offset=None):
        if not isinstance(offset, (int, type(None))): raise TypeError()  # noqa E701
    ...
    def write(self, b):
        if not isinstance(b, (bytes, bytearray, memoryview)): raise TypeError()  # noqa E701
...
class NodeDatabase:
    ...
    def __init__(self, path, secret):
        self.__fs = ConcealFS(path, secret)

    def hierarchy(self):
        for f in NodeDatabase.HIERARCHY:
            self.__fs.tar.add(f)

    def close(self):
        fileobj = self.__fs.tar.fileobj
        self.__fs.tar.close()
        fileobj.close()
...
db = ConcealFS(file, base64.b64decode(secret), 'w')
db.create('lipsum.txt', LIPSUM.encode('utf-8'))
db.create('dolor.txt', LIPSUM.encode('utf-8'))
db.close()

print('############################################')

db = ConcealFS(file, base64.b64decode(secret), 'r')
lip = db.read('lipsum.txt')
dol = db.read('dolor.txt')
db.close()

print(lip.decode('utf8') == LIPSUM)
print(dol.decode('utf8') == LIPSUM)
