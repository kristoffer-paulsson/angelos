...
The boilerplate code provides a dummy ``main`` function that prints out the word 'Hello'::
    ...
    >> from angelos import main
    >> main()
    
When the package is installed via ``easy_install`` or ``pip`` this function will be bound to the ``angelos`` executable in the Python installation's ``bin`` directory (on Windows - the ``Scripts`` directory).
